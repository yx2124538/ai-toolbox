...
- 导入与手动输入模型共用 `fillCodexCatalogModelFromPreset`：以模型 ID 精确匹配预设，补齐空的菜单显示名、上下文窗口、思考档位和默认档位。档位优先读取预设 variants 的 OpenAI `reasoningEffort` / Anthropic `effort` / Gemini `thinkingConfig.thinkingLevel`，仅保留 Codex 支持的档位；旧预设只声明 `reasoning=true` 时沿用手动表单的 `low/high/max` 默认。默认优先 high。新导入行无预设名称时可用 API 名称；已有映射不补写空字段或覆盖自定义值。预设没有声明速度等级，不应据模型名称自动启用 Fast/Ultrafast；保存后重新读取必须保留已有 `serviceTiers`。
...
- 模型映射改动运行 `pnpm test` 和 `pnpm test:codex-model-import`。浏览器检查覆盖预设填充、手动/导入一致性、内置渠道保存及清空、带空格 ID、自定义字段含速度等级的保存再打开、跨搜索多选、显式清理、原生协议切换与过期请求；使用真实表单，Tauri 接口和无关 TOML 编辑器用隔离桩替代。
