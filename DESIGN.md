...
高信息密度不等于把按钮压扁。普通文字操作默认使用 30–32px 高度，文字 12–13px，行高 18–20px；上下保留约 5px 的可见留白，左右内边距 10–12px，图标与文字间距 6px。优先使用 Ant Design 默认尺寸，再按页面密度微调字体；不要把 `size="small"` 当作所有业务操作的默认值。

- `size="small"` 仅用于确实受表格行高或密集工具栏约束的操作。若 24px 按钮承载 14px 文字、导致字形接近上下边框，应增高按钮或缩小字体，不能只把文字塞进既有高度。
- “添加规则”“添加条目”等次级操作默认按内容宽度展示、靠近所属列表；不要因为父容器是纵向 flex 就拉伸为整行细长边框按钮。只有真正需要整行点击的选择项、移动端主操作等场景才使用全宽按钮。
- “管理规则”“查看详情”等分区入口优先使用小图标配文字的轻量按钮，保留 hover、键盘 focus 和 disabled 状态；不必全部包成普通灰边框按钮。
- 删除、更多等图标操作保持 28–32px 的稳定命中区域。hover 不改变宽高、padding 或字重，避免布局跳动。
- 圆角、颜色、边框和交互状态继续复用现有主题 token，不增加装饰性渐变或阴影。不要为单个页面覆盖整站 Ant Design 按钮样式。

局部普通操作按钮可使用以下尺寸基线，按钮颜色和状态继续交给现有主题：
...
固定选项的下拉框必须容纳最长选项文字、箭头和内边距，例如“正则表达式”不能显示成“正则表…”。优先给控件足够的最小宽度，窄窗口再换行排列；省略号只用于长度不可预测的数据值，并提供完整内容的查看方式。

同一字段的标题与辅助说明放在同一文字组，内部使用 2–4px 间距；字段组之间才使用 8–12px 间距。不要将说明拆成独立布局行后重复叠加 section gap，造成标题和说明脱离。

操作区下方的说明属于独立内容组，与上方按钮或工具栏保留 8–12px 的外部间距，默认使用 12px。按钮内部 padding 不能替代这段间距；也不能套用标题与副标题之间的 2–4px。尤其要检查 Tabs 内容末尾的按钮与 Tabs 外说明，二者不在同一个 gap 容器内时需显式设置间距，不能让说明紧贴按钮边框。

验收除检查“不溢出”外，还要在真实父级设置分区或工具栏中检查按钮文字留白、固定标签完整显示、内容宽度按钮、亮暗主题和窄窗口；孤立组件截图不能代替真实容器检查。
