...
const fixtureDirectory = path.join(projectRoot, 'web/test/features/coding/codex/components/fixtures');
...
const artifactRoot = await mkdtemp(path.join(outputParent, 'codex-model-import-'));
...
const aliases = [
  ...
  // A fixture built outside the repo must resolve Zustand through its ESM
  // entry; an absolute package-directory alias bypasses conditional exports.
  ...['', '/react', '/vanilla'].map(suffix => ({
    find: new RegExp('^zustand' + suffix + '$'),
    replacement: path.join(projectRoot, 'node_modules/zustand/esm', (suffix.slice(1) || 'index') + '.mjs'),
  })),
...
await writeFile(path.join(artifactRoot, 'entry.jsx'), 'import ' + JSON.stringify(path.join(fixtureDirectory, 'CodexModelImportFixture.jsx').replaceAll('\\', '/')) + ';');
...
try {
  ...
  const checks = await verifyCodexModelImport({ ...browser, baseUrl: 'http://127.0.0.1:' + server.address().port, artifactRoot });
  ...
  console.log(`Passed ${checks.length} Codex model import browser checks.`);
} finally {
