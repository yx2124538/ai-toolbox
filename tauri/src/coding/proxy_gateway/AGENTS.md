...
- Provider body 兼容规则属于 Gateway runtime outbound adapter 层，应基于 `providerType + apiFormat` 做窄范围处理。不要把 DeepSeek/Moonshot/Zai/Doubao/Grok/Longcat/ModelScope/Bailian/Ollama 等供应商专属限制下沉到通用 `transformer` 协议转换语义里。
...
- OpenAI Responses target 当前只实现 HTTP JSON 和 HTTP SSE transport。早期审计里的 “Responses WebSocket Transport” 不是当前 AxonHub `llm/transformer/openai/responses/outbound.go` 的真实能力：该文件没有 `CustomizeExecutor` / websocket / wss 分支。不要为了追旧文档误加一条未经协议定义的 WebSocket 上游路径；只有后续确认 OpenAI Responses 或参考实现重新引入 WebSocket transport，并明确 request/response/SSE 语义后，才新增 executor 级 transport。
- OpenAI Responses `/responses/compact` 是 Responses-only endpoint，不属于聊天协议跨转换矩阵。Gateway runtime 对同协议 OpenAI Responses target 保持原 compact path 直通；如果 provider target protocol 不是 OpenAI Responses，必须在发送上游前拒绝，不能把 compact 请求误当普通 Responses chat input 转到 Anthropic/Chat/Gemini。
...
- Codex → OpenAI Chat 的多 vendor 推理参数矩阵由 runtime outbound adapter 执行，显式 `data.meta.codexChatReasoning` 优先。该配置可声明 `supportsThinking`、`supportsEffort`、`thinkingParam`、`effortParam`、`effortValueMode` 和 `outputFormat`，用于把 Codex/Responses 风格 `reasoning_effort` 映射到 DeepSeek、OpenRouter、SiliconFlow、DashScope/Qwen、Bailian、Kimi/Moonshot、GLM/Zai、MiniMax、Mimo、StepFun 等 Chat 兼容 provider 的 `thinking` / `enable_thinking` / `reasoning_split` / `reasoning.effort` / `reasoning_effort`。缺显式 `codexChatReasoning` 时，provider 专属 fallback 只能由明确的 `providerType` / `apiFormat` 触发；自定义渠道绝不能因为 body `model` 字符串包含 `deepseek`、`qwen`、`minimax`、`mimo` 等名称就套用对应 provider 规则。模型名只允许在已识别 provider 内做能力细分，例如 `providerType=stepfun` 后再用 `2603` 判断是否支持 effort。新增或修复 provider 兼容能力时，必须同步维护 `gateway_provider_profiles.json`、provider 表单保存逻辑和 runtime 回归测试，让用户通过明确预设或显式 meta 选择兼容行为，而不是依赖模型名猜测。
...
- 跨 HTTP 请求的协议补全状态属于 runtime `side_stores`，不属于 transformer：`CodexHistoryStore` 用于 Codex Responses follow-up 只带 `previous_response_id`/`function_call_output` 时补回上轮 assistant call item；`GeminiShadowStore` 用于 Gemini target 多轮 functionResponse 前回放带 `thoughtSignature` 的上轮 model functionCall。side store 必须有容量上限，且只作为兼容缓存，不能成为 provider 配置或请求日志的 Source of Truth。Gemini shadow 只能在请求头、metadata、`previous_response_id` 或 `cachedContent` 等可靠会话线索存在时记录/回放；缺少线索时必须跳过，不能把所有请求归入 `"default"` 会话导致跨会话污染。
