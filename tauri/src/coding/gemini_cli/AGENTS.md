...
- Auto-creating the default Gemini official provider must require an existing persisted `gemini_cli_official_account` record. A local `settings.json`/`.env` OAuth shape alone is not enough; without a persisted account, do not create or show a local official `__local__` provider.
- The `__local__` provider fallback is only for third-party/API-key/gateway local runtime config. Official account management needs a real provider id from SQLite, so never route `__local__` into official-account commands.
